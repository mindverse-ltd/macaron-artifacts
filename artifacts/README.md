...
- Chat and Canvas follow the physical bottom with a critically damped spring that cannot overshoot or reverse when content grows or stops. User scrolling still pauses following; a layout collapse constrains the position to the new bottom. Chat keeps its normal bottom padding without an elastic offset.
- Streaming assistant text and active reasoning share an 800ms incremental fade with no stagger or delayed text delivery. Completed parts and earlier summaries stay static when history opens. Reasoning aligns with the answer text without a pulsing dot, and retains its height limit, edge fades, automatic following, and wheel/touch/keyboard scrolling without a visible scrollbar.
