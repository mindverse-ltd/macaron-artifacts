...
- `$ui4a/ui` contains Button, Field, Card, Badge, Tabs and Disclosure, following the lightweight `ui4a-playground` contract. Compose layouts with native HTML and UnoCSS Wind4 utilities; use `@headlessui/react` and `recharts` directly for additional controls and charts. `$ui4a/chat`, `$ui4a/state` and `$ui4a/fs` are separate host capabilities.
