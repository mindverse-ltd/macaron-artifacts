  ...
  async *run(turn: HarnessTurn): AsyncGenerator<ChatChunk> {
    ...
    try {
      if (turn.enrichment) {
        ...
        } else { const resumed = rpcPayload(await rpc.request('session.resume', { session_id: storedId || nativeId, cwd: turn.cwd, omit_messages: true }, turn.signal)); nativeId = string(resumed.session_id) || nativeId; storedId = storedId || string(resumed.stored_session_id) || nativeId; turn.onNativeSession(encode({ sessionId: nativeId, storedId, profile: profileName(turn.profile) })); }
