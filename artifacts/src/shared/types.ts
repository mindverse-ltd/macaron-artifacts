...
export type MessageData = { artifact: Artifact; command: { toolCallId: string; output: string }; approval: Approval & { resolved?: boolean }; usage: { inputTokens?: number; outputTokens?: number; cachedInputTokens?: number }; recap: { title?: string; suggestions: string[] }; notice: { message: string } };
