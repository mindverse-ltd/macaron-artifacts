...
export default function App() {
  ...
  return <main className="@container/shell flex h-dvh flex-col overflow-x-clip">
    <header className="theme-titlebar flex h-12 shrink-0 items-center gap-2 border-b border-border px-3"><button type="button" onClick={() => setSidebarOpen(true)} title="会话" aria-label="打开会话列表" className="btn-icon size-8 shrink-0 rounded-lg @[960px]/shell:hidden"><Icon name="menu" /></button><span className="min-w-0 flex-1 truncate text-sm font-medium">{session?.title ?? 'Macaron Artifacts'}</span>
      ...
      <button type="button" onClick={() => setAppearanceOpen(true)} title="外观设置" aria-label="外观设置" aria-haspopup="dialog" className="btn-icon size-8 shrink-0 rounded-lg"><Icon name={preferences.mode === 'system' ? 'monitor' : appearance.dark ? 'moon' : 'sun'} /></button><button type="button" onClick={toggleCanvas} aria-pressed={canvasOpen} className={`interactive shrink-0 rounded-lg px-2 py-1 text-xs ${canvasOpen ? 'bg-surface-3 text-hover-fg' : 'text-muted hover:bg-surface-3 hover:text-hover-fg'}`}>Canvas</button>
    </header>
