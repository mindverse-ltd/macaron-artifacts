...
export default function App() {
  ...
  const canvasToggle = useRef<HTMLButtonElement>(null);
  ...
  return <main className="@container/shell flex h-dvh flex-col overflow-x-clip">
    <header className="theme-titlebar flex h-12 shrink-0 items-center gap-2 border-b border-border px-3"><button type="button" onClick={() => setSidebarOpen(true)} title="会话" aria-label="打开会话列表" className="btn-icon size-8 shrink-0 rounded-lg @[960px]/shell:hidden"><Icon name="menu" /></button><span className="min-w-0 flex-1 truncate text-sm font-medium">{session?.title ?? 'Macaron Artifacts'}</span>
      ...
      <button type="button" onClick={() => setAppearanceOpen(true)} title="外观设置" aria-label="外观设置" aria-haspopup="dialog" className="btn-icon size-8 shrink-0 rounded-lg"><Icon name={preferences.mode === 'system' ? 'monitor' : appearance.dark ? 'moon' : 'sun'} /></button><button ref={canvasToggle} type="button" onClick={toggleCanvas} aria-pressed={canvasOpen} className={`pressable interactive shrink-0 rounded-lg px-2 py-1 text-xs active:scale-[0.98] ${canvasOpen ? 'bg-surface-3 text-hover-fg' : 'text-muted hover:bg-surface-3 hover:text-hover-fg'}`}>Canvas</button>
    </header>
    ...
    <div className="flex min-h-0 flex-1"><Sidebar {...sidebar} /><SidebarDrawer {...sidebar} open={sidebarOpen} onClose={() => setSidebarOpen(false)} />
      <div ref={split.container} className="@container/panes flex min-h-0 min-w-0 flex-1" style={{ ['--canvas-w' as string]: `${(split.fraction * 100).toFixed(2)}%` }}>
        ...
        {canvasOpen ? <aside className="theme-panel min-w-0 w-full @[681px]/panes:w-[clamp(300px,var(--canvas-w),calc(100%-381px))]">{session ? <ArtifactPanel key={session.id} session={session} artifacts={artifacts} selected={selectedArtifact} onSelect={path => actions.openArtifact(session.id, path)} onClose={() => { canvasToggle.current?.focus({ preventScroll: true }); closeCanvas(); }} onSend={text => actions.send(session.id, text)} /> : <div className="flex h-full flex-col"><header className="flex h-12 items-center justify-between px-3 text-sm">Canvas<button type="button" onClick={() => { canvasToggle.current?.focus({ preventScroll: true }); closeCanvas(); }} aria-label="关闭 Canvas"><Icon name="x" /></button></header><p className="p-6 text-xs text-muted">创建会话后，生成的界面会显示在这里。</p></div>}</aside> : null}
      </div>
    </div>
