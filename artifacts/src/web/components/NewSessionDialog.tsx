...
export function NewSessionDialog({ harnesses, initialHarness, initialCwd, onClose, onCreate }: { harnesses: HarnessInfo[]; initialHarness?: HarnessId; initialCwd: string; onClose: () => void; onCreate: (input: { harness: HarnessId; cwd: string; model?: string }) => Promise<void> }) {
  ...
  return <Dialog open onClose={() => { if (!busy) onClose(); }} className="relative z-50"><div className="fixed inset-0 bg-black/40" /><div className="fixed inset-0 flex items-center justify-center p-3 sm:p-4"><DialogPanel className="max-h-[calc(100dvh-2rem)] w-full max-w-lg overflow-y-auto rounded-2xl bg-surface p-4 shadow-2xl sm:p-5">
    ...
    <form aria-busy={busy} onSubmit={event => { event.preventDefault(); void submit(); }} className="flex flex-col gap-4">
      <RadioGroup name="harness" value={harness} onChange={setHarness} disabled={busy} aria-orientation="horizontal" className="flex flex-col gap-2">
      ...
      <Field name="model" label="模型" disabled={busy} value={model} onChange={event => setModel(event.target.value)} placeholder="使用 harness 默认模型" hint="留空时继承这个 harness 的配置。" />
