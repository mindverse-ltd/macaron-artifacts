...
export function Slider({ label, hint, className = '', showValue = true, ...props }: SliderProps) {
  const hintId = useId();
  // Base UI fixes control mode at mount; a streamed value prop can arrive later.
  return <BaseSlider.Root key={props.value === undefined ? 'uncontrolled' : 'controlled'} {...props} className={`ui4a-slider ${className}`}>
...
export function NumberField({ label, hint, className = '', id, placeholder, inputMode = 'decimal', incrementLabel = '增加', decrementLabel = '减少', ...props }: NumberFieldProps) {
  ...
  // Reset only on control-mode changes, so later streamed values are not ignored.
  return <BaseNumberField.Root key={props.value === undefined ? 'uncontrolled' : 'controlled'} {...props} id={inputId} className={`ui4a-number ${className}`}>
