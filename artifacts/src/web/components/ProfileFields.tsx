...
function CredentialFields({ harness, config, onChange, credentials, configured, onCredentialsChange, disabled }: Omit<ProfileFieldsProps, 'options'>) {
  const authMode = config.authMode ?? 'inherit', kind = authMode === 'auth-token' ? 'authToken' : 'apiKey';
  ...
  return <div className="grid gap-4 sm:grid-cols-2"><Choice label="认证方式" value={authMode} onChange={value => onChange({ ...config, authMode: value as ProfileConfig['authMode'] })} options={[{ value: 'inherit', label: '继承本机认证' }, { value: 'api-key', label: 'API Key' }, ...(harness === 'claude-code' ? [{ value: 'auth-token', label: 'Bearer Token' }] : [])]} hint="本机登录凭据和环境变量保持不变。" disabled={disabled} />
...
export function ProfileFields(props: ProfileFieldsProps) {
  ...
  return <div className="space-y-5">
    <div className="grid gap-4 sm:grid-cols-2"><FreeChoice label="主模型" value={config.model} options={models} onChange={value => update({ model: optional(value) })} hint="搜索已知模型，或直接输入模型 ID。" disabled={disabled} />
      ...
      {harness === 'opencode' ? <FreeChoice label="主代理" value={config.agent} options={(options.agents ?? []).filter(agent => !agent.subagent).map(agent => ({ value: agent.id, label: agent.name }))} onChange={value => update({ agent: optional(value) })} disabled={disabled} /> : null}
    </div>
    <div className="grid gap-4 sm:grid-cols-2">{harness !== 'claude-code' ? <FreeChoice label="Provider" value={config.provider} options={asOptions(options.models.flatMap(model => model.provider ? [model.provider] : []))} onChange={value => update({ provider: optional(value) })} placeholder="从所选模型推断" disabled={disabled} /> : null}<Field label="API 地址" type="url" inputMode="url" autoComplete="off" value={config.baseUrl ?? ''} onChange={event => update({ baseUrl: optional(event.target.value) })} placeholder="继承原生服务地址" hint="自定义网关或兼容服务的 Base URL。" disabled={disabled} /></div>
