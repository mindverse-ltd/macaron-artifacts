...
export function ProfileFields(props: ProfileFieldsProps) {
  ...
  return <div className="space-y-5">
    <div className="grid gap-4 sm:grid-cols-2"><FreeChoice label="主模型" value={config.model} options={models} onChange={value => update({ model: optional(value) })} hint="搜索已知模型，或直接输入模型 ID。" disabled={disabled} />
      ...
      {(harness === 'opencode' || harness === 'opencode-v2') || harness === 'openclaw' ? <FreeChoice label="主代理" value={config.agent} options={(options.agents ?? []).filter(agent => !agent.subagent).map(agent => ({ value: agent.id, label: agent.name }))} onChange={value => update({ agent: optional(value) })} disabled={disabled} /> : null}
    </div>
    ...
    {(harness === 'opencode' || harness === 'opencode-v2') ? <AgentModels config={config} options={options} onChange={onChange} disabled={disabled} models={models} /> : null}
