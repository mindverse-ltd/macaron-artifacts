...
export function Composer({ disabled, busy, onSend, onStop }: { disabled: boolean; busy: boolean; onSend: (text: string) => void; onStop: () => void }) {
  ...
  return <div className="shrink-0 border-t border-border p-3"><div className="interactive mx-auto flex w-full max-w-3xl items-end gap-2 rounded-2xl border border-control-border p-2 focus-within:border-focus">
    ...
    {busy ? <button type="button" onClick={onStop} title="停止" aria-label="停止" className={`interactive grid size-8 shrink-0 place-items-center rounded-full ${sendable ? 'text-muted hover:bg-surface-3 hover:text-fg' : 'bg-accent text-accent-fg hover:bg-accent-hover active:scale-95'}`}><Icon name="x" filled className="size-3.5" /></button> : null}
