...
export function Conversation({ instance, session, store }: { instance: Chat<ChatMessage>; session: SessionSummary; store: WorkspaceStore }) {
  ...
  return <div className="@container relative flex h-full min-w-0 flex-1 flex-col">
    ...
    <div className="px-3"><div className="mx-auto w-full max-w-3xl">
      ...
      {suggestions.length ? <div className="mb-2 flex flex-wrap gap-2">{suggestions.map(item => <Button key={item} variant="ghost" size="sm" onClick={() => send(item)} className="suggestion h-auto max-w-full rounded-full border border-contrast bg-surface-2 py-1.5 text-left font-normal whitespace-normal break-words">{item}</Button>)}</div> : null}
    </div></div>
...
const Message = memo(function Message({ message, streaming, sessionId, cwd, onSend, onArtifact, onApprove }: { message: ChatMessage; streaming: boolean; sessionId: string; cwd: string; onSend: (text: string) => void; onArtifact: (path: string) => void; onApprove: (id: string, approved: boolean) => Promise<unknown> }) {
  ...
  return <article data-message-role={message.role} className={message.role === 'user' ? 'theme-bubble max-w-[85%] self-end rounded-2xl px-4 py-2 text-sm' : 'flex flex-col gap-3'}>
    {message.parts.map((part, index) => {
      ...
      if (part.type === 'reasoning') return <details key={index} className="overflow-clip rounded-lg border border-contrast text-xs text-muted"><summary className="interactive cursor-pointer px-3 py-2 select-none hover:bg-surface-3 hover:text-hover-fg">思考过程</summary><Collapsible className="theme-code"><p className="px-3 py-2 leading-relaxed whitespace-pre-wrap text-code-fg">{part.text}</p></Collapsible></details>;
