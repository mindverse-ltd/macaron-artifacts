...
export function Conversation({ instance, session, store }: { instance: Chat<ChatMessage>; session: SessionSummary; store: WorkspaceStore }) {
  ...
  return <div className="@container relative flex h-full min-w-0 flex-1 flex-col">
    ...
    <button type="button" onClick={() => scrollToBottom()} inert={stuck} className={`interactive absolute bottom-24 left-1/2 z-floating-control -translate-x-1/2 theme-widget rounded-full px-3 py-1.5 text-xs text-muted hover:text-fg ${stuck ? 'translate-y-1 opacity-0' : 'opacity-100'}`}>回到底部 ↓</button>
