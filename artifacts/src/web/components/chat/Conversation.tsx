...
export function Conversation({ instance, session, store }: { instance: Chat<ChatMessage>; session: SessionSummary; store: WorkspaceStore }) {
  ...
  return <div className="@container relative flex h-full min-w-0 flex-1 flex-col">
    ...
    <SessionComposer store={store} sessionId={session.id} busy={streaming} onSend={send} scrolledAway={!stuck} onScrollToBottom={scrollToBottom} />
  </div>;
}

function SessionComposer({ store, sessionId, busy, onSend, scrolledAway, onScrollToBottom }: { store: WorkspaceStore; sessionId: string; busy: boolean; onSend: (text: string) => void; scrolledAway: boolean; onScrollToBottom: (behavior: ScrollBehavior) => void }) {
