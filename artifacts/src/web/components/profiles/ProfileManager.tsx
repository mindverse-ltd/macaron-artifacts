  ...
  return <>
    <Dialog open onClose={() => navigate(onClose)} className="relative z-50"><div className="fixed inset-0 bg-black/40" /><div className="fixed inset-0 flex items-center justify-center p-3 sm:p-6"><DialogPanel className="theme-widget flex max-h-[calc(100dvh-1.5rem)] w-full max-w-3xl flex-col overflow-hidden rounded-2xl sm:max-h-[calc(100dvh-3rem)]">
      ...
      <TabGroup selectedIndex={Math.max(0, harnesses.findIndex(item => item.id === harness))} onChange={selectHarness} className="mt-4 flex min-h-0 flex-1 flex-col">
        <TabList aria-label="Harness Profiles" className="mx-5 grid shrink-0 grid-cols-4 gap-1 rounded-lg bg-surface-2 p-1 sm:mx-6">{harnesses.map(item => <Tab key={item.id} disabled={busy} className="interactive flex min-w-0 flex-col items-center justify-center gap-1 rounded-md px-1 py-2 text-[11px] sm:flex-row sm:gap-1.5 text-muted data-[selected]:bg-tab-active data-[selected]:text-tab-active-fg sm:text-sm"><HarnessIcon harness={item.id} /><span className="truncate">{item.name}</span></Tab>)}</TabList>
    ...
    {pending ? <Dialog open onClose={() => setPending(undefined)} className="relative z-70"><div className="fixed inset-0 bg-black/40" /><div className="fixed inset-0 grid place-items-center p-4"><DialogPanel className="theme-widget w-full max-w-sm rounded-2xl p-5"><DialogTitle className="text-base font-medium">放弃未保存的修改？</DialogTitle><p className="mt-2 text-sm text-muted">模型、连接和凭据的修改尚未保存。</p><div className="mt-5 flex justify-end gap-2"><Button autoFocus variant="secondary" onClick={() => setPending(undefined)}>继续编辑</Button><Button variant="danger" onClick={() => { const action = pending; setPending(undefined); setDirty(false); action(); }}>放弃修改</Button></div></DialogPanel></div></Dialog> : null}
  </>;
}
...
function ProfileEditor({ harness, cwd, profile, busy, onBusy: setBusy, onDirty, onSaved, onDeleted }: { harness: HarnessId; cwd: string; profile?: HarnessProfile; busy: boolean; onBusy(busy: boolean): void; onDirty(dirty: boolean): void; onSaved(profile: HarnessProfile): void; onDeleted(): void }) {
  ...
  return <><form onSubmit={event => { event.preventDefault(); void submit(); }} aria-busy={busy} className="mt-5 border-t border-contrast pt-5">
    ...
    <Dialog open={deleting} onClose={() => { if (!busy) setDeleting(false); }} className="relative z-70"><div className="fixed inset-0 bg-black/40" /><div className="fixed inset-0 grid place-items-center p-4"><DialogPanel className="theme-widget w-full max-w-sm rounded-2xl p-5"><DialogTitle className="text-base font-medium">删除「{profile?.name}」？</DialogTitle><p className="mt-2 text-sm text-muted">{profile?.source === 'codex' ? '对应的 Codex 原生 Profile 文件也会被删除。' : '配置及其保存的凭据将被移除。'}</p><div className="mt-5 flex justify-end gap-2"><Button autoFocus variant="secondary" disabled={busy} onClick={() => setDeleting(false)}>取消</Button><Button variant="danger" disabled={busy} onClick={() => { void destroy(); }}>{busy ? '正在删除…' : '确认删除'}</Button></div></DialogPanel></div></Dialog>
  </>;
}
