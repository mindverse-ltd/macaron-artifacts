...
function describeRun(h) {
    ...
    if (h.url)
        return h.url;
    if (h.server || h.tool)
        return [h.server, h.tool].filter(Boolean).join(' · ');
    if (h.prompt)
        return h.prompt;
    if (h.agent)
        return h.agent;
...
function flattenSettings(raw, scope, source) {
    const out = [];
...
async function readSettingsFile(p) {
    let raw;
    try {
    ...
    try {
        return { kind: 'ok', settings: JSON.parse(raw) };
    }
    catch (e) {
        return { kind: 'parseError', error: e.message };
    }
}
...
export async function readHooks(cwd) {
    const wanted = sourcePaths(cwd);
    const results = await Promise.all(wanted.map(async ({ scope, path: p }) => {
...
//# sourceMappingURL=hooks-store.js.map