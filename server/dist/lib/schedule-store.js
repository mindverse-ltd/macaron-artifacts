...
export function computeNextRun(pattern, from = new Date()) {
...
export async function warmSchedulesCache() {
    await readSchedules();
}
...
export async function createSchedule(input) {
    ...
    if (input.oneShot && nextRunAt === null)
        throw new Error('one-time schedule is in the past');
    const sched = {
        id: randomUUID(),
        name: input.name,
        prompt: input.prompt,
...
export async function updateSchedule(id, patch) {
    ...
    if (needsRearm && s.status === 'active' && nextOneShot && recomputed === null)
        throw new Error('one-time schedule is in the past');
...
export async function recordRun(id, result, advanceNext = true) {
    const s = (await readSchedules()).find((x) => x.id === id);
    if (!s)
        return;
    const now = Date.now();
    ...
    if (result.sessionId)
        s.lastSessionId = result.sessionId;
    if (!advanceNext) {
        s.updatedAt = now;
        await persist();
        return;
