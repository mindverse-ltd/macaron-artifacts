...
async function drain(schedule, stream) {
    ...
    try {
        for await (const ev of stream) {
            if (ev.kind === 'session' && !sid) {
                ...
                if (sid)
                    livePush(sid, { type: 'delta', text: ev.text });
            }
            else if (ev.kind === 'tool_use') {
                if (sid)
                    livePush(sid, { type: 'tool_use', id: ev.id, name: ev.name, input: ev.input });
            }
            else if (ev.kind === 'tool_input_delta') {
                if (sid)
                    livePush(sid, { type: 'tool_input_delta', id: ev.id, name: ev.name, partial_json: ev.partial_json, accumulated: ev.accumulated });
            }
            else if (ev.kind === 'tool_input_done') {
                if (sid)
                    livePush(sid, { type: 'tool_input_done', id: ev.id, name: ev.name, final_json: ev.final_json });
            }
            else if (ev.kind === 'tool_result') {
                if (sid)
                    livePush(sid, { type: 'tool_result', tool_use_id: ev.tool_use_id, text: ev.text, isError: ev.isError });
            }
            else if (ev.kind === 'usage') {
                if (sid)
                    livePush(sid, { type: 'usage', outputTokens: ev.outputTokens, thinkingTokens: ev.thinkingTokens });
            }
            else if (ev.kind === 'message') {
                if (sid)
                    livePush(sid, { type: 'event', event: 'system', subtype: ev.subtype });
            }
            else if (ev.kind === 'error') {
                ok = false;
                if (sid)
                    livePush(sid, { type: 'error', error: ev.error });
            }
            else if (ev.kind === 'done') {
                sawDone = true;
                if (ev.exitCode !== 0)
                    ok = false;
                if (sid) {
                    liveEnd(sid, { type: 'done', exitCode: ev.exitCode });
                    endRun(sid);
                }
            }
        }
        ...
        if (sid && !sawDone) {
            if (isClaude)
                liveEnd(sid, { type: 'done', exitCode: -1 });
            endRun(sid);
        }
    }
...
export async function fireSchedule(schedule, advance = true) {
    if (inFlight.has(schedule.id))
        return { ok: false, sessionId: null, error: 'schedule already running' };
    ...
    try {
        ...
        await recordRun(schedule.id, { sessionId: null, ok: false }, advance).catch((e) => console.error(`[scheduler] recordRun failed for ${schedule.id}:`, e.message));
...
function tick() {
    ...
    for (const s of listSchedules()) {
        if (s.status !== 'active' || s.nextRunAt === null || s.nextRunAt > now)
            continue;
