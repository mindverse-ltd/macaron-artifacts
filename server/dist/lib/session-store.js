...
function isNoisyUserText(t) {
    ...
    if (t.startsWith('<'))
        return true;
    if (/^The file .* (has been (updated|created) successfully|file state is current)/.test(t))
        return true;
    if (/^Tool .* failed/.test(t))
        return true;
    return false;
}
...
export async function searchMessages(query, limit = 30) {
    ...
    for (const s of sessions) {
        ...
        for (const line of raw.split('\n')) {
            ...
            try {
                const o = JSON.parse(line);
                if ((o.type !== 'user' && o.type !== 'assistant') || o.isMeta)
                    continue;
                ...
                // renders — a dead link.
                if (o.type === 'user' && isNoisyUserText(text))
                    continue;
                ...
                if (!text || text.toLowerCase().indexOf(ql) < 0)
                    continue;
                hits.push({
                    project: s.project,
