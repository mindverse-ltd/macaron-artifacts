...
export async function warmWorktreeCache() {
    await records();
}
// --- git helpers -------------------------------------------------------
...
export async function createWorktree(baseCwd) {
    if (!(await isGitWorkTree(baseCwd)))
        return null;
    ...
    return withRepoLock(repoRoot, async () => {
        ...
        if (baseBranch === 'HEAD')
            throw new Error('cannot isolate: base repo is in detached HEAD state (check out a branch first)');
...
export async function bindWorktree(sessionId, p) {
...
export async function cleanupPendingWorktree(p) {
...
export async function getWorktree(sessionId) {
...
export async function mergeWorktree(sessionId) {
    ...
    if (!r)
        throw new WorktreeError('no active worktree for this session');
    if (!(await exists(r.worktreePath)))
        throw new WorktreeError('worktree directory is gone', true);
    if (await isDirty(r.worktreePath))
        throw new WorktreeError('worktree has uncommitted changes — commit them first', true);
...
export async function discardWorktree(sessionId, force = false) {
    const list = await records();
    const r = list.find((x) => x.sessionId === sessionId && x.status === 'active');
    if (!r)
        throw new WorktreeError('no active worktree for this session');
