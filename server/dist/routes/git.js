...
async function resolveCwd(project, sid) {
...
export async function registerGitRoutes(app) {
    // Branch/ahead/dirty/existing-PR snapshot used to prefill and gate the
    // Create-PR dialog.
    app.get('/api/sessions/claude/:project/:sid/pr-context', async ({ params }, reply) => {
    ...
    app.post('/api/sessions/claude/:project/:sid/pr', async (req, reply) => {
