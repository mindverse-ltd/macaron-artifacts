...
export const createTypeCheckService = (ts: typeof TypeScript, { root, filename, compilerOptions, ambient }: { root: string; filename: string; compilerOptions: TypeScript.CompilerOptions; ambient?: string }): TypeCheckService => {
  ...
  const snapshotCache = new Map<string, TypeScript.IScriptSnapshot>();
  const host: TypeScript.LanguageServiceHost = {
    ...
    getScriptSnapshot: (fileName) => {
      ...
      if (resolved === appFile) return ts.ScriptSnapshot.fromString(serviceState.appSource);
      if (resolved === ambientFile) return ambient === undefined ? undefined : ts.ScriptSnapshot.fromString(ambient);
      let cached = snapshotCache.get(resolved);
      if (!cached) { const text = ts.sys.readFile(fileName); if (text !== undefined) { cached = ts.ScriptSnapshot.fromString(text); snapshotCache.set(resolved, cached); } }
      return cached;
    },
