...
export async function getGenuiSystemPrompt(): Promise<string> {
  const ttl = 10 * 60_000;
  if (cachedPrompt && Date.now() - cachedPromptAt < ttl) return cachedPrompt;
  try {
...
const DISPLAY_TSX_TOOL = {
  ...
  function: {
    name: 'display_tsx',
    description:
      'Render a streaming TSX module inline. The `code` field MUST start with imports and then `export default function App()`. ' +
 ...
 */
export async function streamTsx(
  prompt: string,
...
function extractCodePrefix(raw: string): string {
  const m = /"code"\s*:\s*"((?:\\.|[^"\\])*)/.exec(raw);
  if (!m) return '';
  return unescapeJsonString(m[1]!);
}

function parseFinalCode(toolArgs: string): string {
  try {
  ...
  if (!m) throw new Error('Macaron did not produce a `code` field. Args: ' + toolArgs.slice(0, 200));
  return unescapeJsonString(m[1]!);
}
 ...
 */
export async function generateTsx(prompt: string, signal?: AbortSignal): Promise<string> {
