...
export async function registerCodexRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post<{ Body: NewThreadBody }>('/api/codex/threads', async (req, reply) => {
    ...
    pipeCodexToSSE(reply, wrapped as ReturnType<typeof runCodex>, null, { cwd, text, hasImages: images.length > 0 }, runId);
  });
  ...
  app.post<{ Params: SidParams; Body: MessageBody }>(
    ...
    async (req, reply) => {
      ...
      const abortController = new AbortController();
      registerRun(sid, abortController, runId);
      pipeCodexToSSE(reply, runCodexTurn({ prompt: text, cwd, resume: sid, images, abortController, runtime: pickRuntimeOverride(req.body), runId }), sid, { cwd, text, hasImages: images.length > 0 }, runId);
    },
  );
