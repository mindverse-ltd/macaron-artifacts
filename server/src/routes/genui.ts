...
const DISPLAY_TSX_TOOL = {
  ...
  function: {
    name: 'display_tsx',
    description:
      'Render a streaming TSX module inline. The host parses progressively — earlier-arriving JSX paints first. ' +
...
let cachedGenuiPrompt: string | null = null;
let cachedGenuiPromptAt = 0;

async function getGenuiSystemPrompt(): Promise<string> {
  const ttl = 10 * 60_000;
  if (cachedGenuiPrompt && Date.now() - cachedGenuiPromptAt < ttl) return cachedGenuiPrompt;
  try {
...
export async function registerGenuiRoutes(app: FastifyInstance): Promise<void> {
  app.post<{ Body: Body }>('/api/genui', async (req, reply) => {
    ...
    try {
      ...
      reply.raw.write(`data: ${JSON.stringify({ error: `upstream fetch failed: ${(e as Error).message}` })}\n\n`);
      reply.raw.end();
      return;
    }
    ...
    try {
      ...
      reply.raw.write(`data: ${JSON.stringify({ error: `stream error: ${(e as Error).message}` })}\n\n`);
    }
    reply.raw.end();
  });
}
