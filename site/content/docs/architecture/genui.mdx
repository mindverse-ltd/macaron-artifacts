...
<Callout type="warn" title="Availability depends on the install form">
  This describes the retained source application. Its semantic TypeScript diagnostics now resolve `@genui/ui` declarations from the installed package. The unified `macaron-artifacts` distribution has its own harness and UI4A pipeline; see [Packaging](/docs/architecture/packaging).
</Callout>
...
On the browser side, `web/src/components/GenuiRenderer.tsx` connects the host import map to published `partial-react` and the shared frame-delivery implementation. It tolerates **incomplete** source, so each streamed chunk can render without waiting for the full module. Components and charts come from `@genui/ui`; no renderer or component implementation is copied into this repository.
