...
<Cards>
  ...
  <Card title="Codex side">
    Exposed by a standalone stdio MCP server (`macaron-mcp-stdio.ts`) injected into every Codex turn, so `render_ui` is available over the `codex app-server` transport too.
  </Card>
</Cards>
