...
- **web** — `index.html` is the retained Claude SPA entry, `codex.html` the Codex one, and `kimi.html` the Kimi one. They use the published `@genui/ui` component library and `partial-react` renderer, with a small host bridge in `src/components/GenuiRenderer.tsx`.
