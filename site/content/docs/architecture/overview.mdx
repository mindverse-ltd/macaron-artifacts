...
- **web** — `index.html` is the retained Claude SPA entry, `codex.html` the Codex one, and `kimi.html` the Kimi one. They use the published `partial-react` renderer and share the unified app's six lightweight `$ui4a/ui` components, with UnoCSS Wind4 styling, direct Headless UI and Recharts imports, and a small host bridge in `src/components/GenuiRenderer.tsx`.
...
<Cards>
  ...
  <Card title="GenUI" href="/docs/architecture/genui">
    The `render_ui` tool, partial-react renderer, and shared lightweight UI4A components that render model-authored TSX as tokens stream.
  </Card>
  <Card title="Packaging" href="/docs/architecture/packaging">
    The unified package, prebuilt bundles, and the pkg.pr.new per-commit release flow.
  </Card>
</Cards>
