...
`bunx` resolves by bin name, so `mcx@…` boots the Codex launcher from the same `mcc` package. Replace `<sha>` with a commit on `main`. Both bins accept `--host` / `--port`; run `--help` for all flags.
